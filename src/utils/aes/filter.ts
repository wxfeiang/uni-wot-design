...
const API_ENCRYPT_WHITE = ['*.js', '*.css', '*.json', '*.html', '*.png', '.jpg', '.gif']
  ...
  const utils = {
    ...
    onRequest: (method: any) => {
      ...
      if (!method?.meta?.ignorEencrypt) {
        if (method.type === 'GET' && method.url.indexOf('?') > -1) {
          ...
          // 处理 body 为字符串的参数
          if (body && typeof body === 'string' && !utils.isJSON(body)) {
            method.data = utils.changeJsonParam(method.body, method.url, aesKey)
          }
        ...
        // 处理 params
        if (method.url.indexOf('?') === -1 && Object.keys(config.params).length > 0) {
    ...
    changeJsonParam: (body: any, url: string, aesKey: string) => {
      ...
      if (params) {
        ...
        for (const k in params) {
          bodyStr +=
            (bodyStr.indexOf('=') !== -1 ? '&' : '') + k + '=' + encodeURIComponent(params[k])
        }
      }
      return bodyStr
    },
