...
// globs
import { qrOptsDefault } from './interface'
...
export async function qr(ctx, option, canvas2d) {
  ...
  const borderWidth = size * defaults.border
  const qrSize = size - borderWidth * 2
  const backgroundColor = defaults.backgroundColor
  const backgroundImage = defaults.backgroundImage
  const logoImage = defaults.logoImage
  let i
  let j
  let points
  let width
  ...
  let px
  let logoPos
  const linearDir = defaults.linearDir
