...
const { sendGetStdTDCode, encrypt } = useCodeServe() // countdown
  ...
  <view v-if="!show" class="min-h-100vh bg-#F2F3F7">
    <view class="back">
      ...
      <view class="flex items-center justify-around gap-10px my-20px px-15px">
        ...
        >
          <wd-img :src="item.logo" width="28" height="28" v-if="item.logo"></wd-img>
          ...
          <view class="bg-#fff py-20px rounded-10px overflow-hidden min-h-250px">
            ...
            <template v-else>
              <view class="px-20px">
                ...
                <!-- <view class="my-20px">
                  <wd-button block :round="false" disabled>即开通</wd-button>
                </view> -->
              </view>
            </template>
          </view>
